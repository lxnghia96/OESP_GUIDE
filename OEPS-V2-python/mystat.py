...
usb_pid = "0x0003" # Default USB product ID, can also be adjusted in the GUI
...
def make_label_entry(parent, labelname):
    ...
    return entry
def make_combo_box(parent, labelname):
    """Make a labelled input field for parameter input."""
    hbox = QtGui.QHBoxLayout()
    label = QtGui.QLabel(text=labelname)
    entry = QtGui.QComboBox()
    usb_list = usb.core.find(find_all=True)
    for cfg in usb_list:
        if hex(cfg.idVendor) == 0xa0a0:
            entry.addItem(hex(cfg.idProduct))
    hbox.addWidget(label)
    hbox.addWidget(entry)
    parent.addLayout(hbox)
    return entry
...
hardware_usb_pid = make_combo_box(hardware_usb_vid_pid_layout, "PID")
...
hardware_calibration_shuntvalues = [make_label_entry(hardware_calibration_shunt_resistor_layout, "R%d"%i) for i in range(1,5)]
for i in range(0,4):
